...
| # | 问题 | 严重度 | 类别 |
|---|------|:------:|------|
| P0-1 | 回测使用固定评分 FIXED_SCORE_MAP（前视偏差） | 🔴 致命 | 回测可信性 |
| P0-2 | 回测股票池 FIXED_UNIVERSE 幸存者偏差 | 🔴 致命 | 回测可信性 |
| P0-3 | 信号当日收盘价成交，无 T+1 | 🔴 高 | 回测可信性 |
| P0-4 | v3 引擎 `_decide_loop` 卖出无成本模型 | 🔴 高 | 回测可信性 |
| P0-5 | `/api/comparison` 脏数据（卖价=买价、pnl=0） | 🔴 高 | 数据正确性 |
| P0-6 | 净值曲线日期重复 + 非每日市值 | 🔴 高 | 数据正确性 |
| P0-7 | 止损线假数（峰值追踪未生效） | 🔴 高 | 数据正确性 |
| P0-8 | 证据链与因子分自相矛盾 | 🟠 中 | 数据正确性 |
| P0-9 | 胜率口径三套并存 | 🟠 中 | 口径一致性 |
| P0-10 | sortino 缺失时 fallback 到 score | 🟠 中 | 指标正确性 |
| P0-11 | 假进度条 | 🟠 中 | 产品化 |
| P0-12 | `alert()`/`window.open()` 脚本级交互 | 🟡 低 | 产品化 |
| P0-13 | 120 天报告滚动指标图空 + x 轴日期损坏（窗口短） | 🔴 高 | 回测可信性 |
| P0-13b | compare 报告净值对比图缺失（svg=0） | 🔴 高 | 回测可信性 |
| P0-14 | 前端宣传图表口径（1000 天齐全 / 120 天残缺 / VaR 无独立图） | 🟠 中 | 产品化 |
| P0-15 | 图表裁切（所有报告）+ deepseek-vision 精度局限 | 🟡 低 | 产品化 |
