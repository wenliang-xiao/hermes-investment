...
## 还欠缺的内容（2026-09-01 核查，按严重程度）

### 🔴 P0 级（致命，阻塞对外宣称回测数字）

**P0-1 前视偏差最终方案未实现**——时点评分（point-in-time）尚未实现，`score_map = dict(FIXED_SCORE_MAP)` 仍是全程固定评分。告警条无法摘除，CAGR 60% 仍是前视失真假象。

**P0-2 幸存者偏差最终方案未实现**——股票池重建（纳入历史剔除/退市标的）尚未做，19 只龙头池子的 +34.77% 超额收益仍是"选对池子"的假象。

### 🟠 P1 级（重要，数据正确性/体验）

**持仓因子数据缺失**（P0-8 的残留）——全志科技、长江电力等持仓仍显示"因子视角：无因子数据"。根因：因子只来自 `scan_snapshot_latest`，已持仓但不进当日扫描的标的拿不到因子。证据链修了，但持仓卡片/弹窗的因子雷达数据源未修。

**ETF 组合配置语义混乱 + 无过期检测**——"合并建议"仍是伪合并（择时+非择时权重直接拼接，总和远超 100%）；"511010 国债ETF 卖出 50%" 卖出却带 50% 权重，action/weight 矛盾；数据本次更新是一次性的，无"过期自动重算"机制（详见 `etf-tab-analysis-2026-08-31.md`）。

**P0-15 图表裁切未处理**——1000 天报告的水下图/收益分位数图/滚动 Sharpe 仍被裁切。

**P0-14 实质未解决**——仅改了文案，VaR 仍无独立图、短窗口滚动图仍缺失。

### 🟡 P2 级（次要，产品化）

**信息架构核心重构未做**——仅改了 tab 名（"模拟盘"→"今日概览"），未做 `ia-redesign` 文档的核心项：首屏"结论先行"、合并"执行决策区"+"信号日志"（同一批信号两套视图仍并存）、回测 tab 默认展示缓存结果（打开仍是空态）。

**交易弹窗"规则标签 vs 实际浮亏"区分不清**——"HardSeller(-8%)"是规则标签，实际触发时可能已浮亏 -10.3%，UI 未区分。

**TradingAgents 净值曲线整条消失**——0 笔交易导致 `series` 跳过它，前端图表只有 2 条线，应显示平直的 100 万基准线而非消失。

**旧回测报告未重新生成**——`lds_compare_1000d`（0 svg）、120 天报告（空滚动图）都是旧代码产物，需重新运行回测才能体现 P0-13/13b 的修复效果。
...
```
剩余 P0（硬骨头，回测可信性根本）:
  P0-1 时点评分 → P0-2 股票池重建

剩余 P1（数据正确性）:
  持仓因子数据源 → ETF 组合语义+过期检测 → P0-15 裁切 → P0-14 口径

剩余 P2（产品化）:
  信息架构核心 → 交易弹窗口径 → TradingAgents 曲线 → 旧报告重跑
```
